## PLOT FUNCTION
sectorplot <- function(data, levels, fun=length, 
  sort="alphabetical", sort.level=1:length(levels), 
  sector.size=NA, sector.col=NA, 
  height.inner=0.3, height=(0.7-height.inner)/length(levels), 
  range=NULL, col=c("royalblue","chartreuse3","darkgoldenrod1","red"), 
  col.steps=100, border=NULL, lwd=1, labels=NA, label.col=grey(0.5),
  label.thresh=NA, main=NULL, draw.legend=TRUE) {
  ...
  if (sort=="alphabetical")
    sort.fun <- function(df, level) order(df[[levels[level]]])
  else if (sort=="sector.size") {
    if (is.na(sector.size))
      stop("if ordering by sector.size, a column name must be given")
    else {
      sort.fun <- function(df, level) {
        level.order <- as.numeric(reorder(x=df[[levels[level]]], df[[sector.size]], fun))
  ...
  if (!is.numeric(sort.level) | any(!sort.level %in% 1:length(levels)))
    stop("'sort.level' must be numeric and include at least one level")

  
# CALCULATE SECTOR RANGE AND DRAW SECTOR
  draw.sector <- function(level, lower.bound, upper.bound, 
    color, draw.label, sector.name) {
    ...
    polygon(xx, yy, border=border, col=color,
      lwd=lwd)
    
    # draw labels
    if (draw.label) {
      if (a*cos(median(z)) >=0) side=1 else side= -1
      sinz <- sin(median(z)); cosz <- cos(median(z))
      #draw label arcs
      z <- z[-c(1,length(z))]
      lines(c(0.72*cos(z[1]),0.75*cos(z), 0.72*cos(tail(z,1))), 
        ...
        col=label.col)
      #draw label lines
      lines(x=c(0.75*cosz, 0.75*cosz+0.25*cosz*abs(sinz), 0.8*side), 
        y=c(0.75*sinz, 0.75*sinz+0.25*sinz*abs(sinz), 
          0.75*sinz+0.25*sinz*abs(sinz)),
        col=label.col)
      #draw label text
      text(x=0.8*side, y=0.75*sinz+0.25*sinz*abs(sinz), 
        labels=substring(sector.name, 1, 18), col=label.col, 
        pos=3+side, offset=0.2, cex=0.7)
    }
  }
  ...
  sector.core <- function(level, df, lower.bound) {
    ...
    for (category in unique(df[[levels[level]]])) {
      ...
      if (is.na(sector.size))
        ...
        upper.bound <- lower.bound + fun(df[sector.hits, sector.size])/fun(data[[sector.size]])
      ...
      if (is.na(sector.col))
        ...
        if (expression < range[1]) color <- ramp[1]
        if (expression > range[2]) color <- ramp[col.steps]
      ...
      draw.sector(level, lower.bound, upper.bound, color=color, 
        draw.label, sector.name=category)
      
      # CALL CORE FUNCTION RECURSIVELY
      if (level!=length(levels))
        sector.core(level=level+1, df=df[sector.hits, ],
  ...
  if (draw.legend){
    ...
    if (is.na(sector.col))
      legend(xcoord, ycoord, legend=levels, pch=15, pt.cex=3, 
        col=colorRampPalette(col)(length(levels)), xpd=NA, bty="n")
    else {
      legend.labels <- pretty(range, n=5)
      col=colorRampPalette(col)(length(legend.labels))
      legend(xcoord, ycoord, legend=legend.labels, pch=15, col=col, pt.cex=2, 
        cex=0.6, text.col=grey(0.5), xpd=NA, bty="n")
    }
  }
}
