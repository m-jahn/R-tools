...
apply.norm <- function(
  df, norm.function, sample.cols = 1:ncol(df),
  ref.cols = NULL) {
  ...
  if (norm.function == "normalize.quantiles.in.blocks") {
    ...
    blocks <- apply(df[sample.cols], 1, function(x) median(x, na.rm = TRUE)) %>% 
      quantcut(q = 20)
    df_norm <- df[sample.cols] %>% as.matrix %>%
      normalize.quantiles.in.blocks(blocks)
    
  } else if (norm.function == "justvsn") {
      
    if (is.null(ref.cols)) {
      df_norm <- justvsn(as.matrix(df[sample.cols])) %>% 
        apply(2, function(x) 2^x)
    } else {
      df_norm <- justvsn(as.matrix(df[sample.cols]), 
        reference = vsn2(as.matrix(df[ref.cols]))) %>% 
        apply(2, function(x) 2^x)
    }
      
  } else if (norm.function != "none") {
  ...
  df_norm <- as.data.frame(df_norm) %>% setNames(colnames(df[sample.cols]))
  bind_cols(df %>% select(-sample.cols), df_norm)
}
